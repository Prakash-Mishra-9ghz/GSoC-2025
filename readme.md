...
- **PR #[10631](https://github.com/oss-review-toolkit/ort/pull/10631)**: [scanner: Add Nomos plugin] - [Status: Under Review]  
  - **Description**: Introduced a new external scanner plugin to integrate FOSSology’s Nomos agent into ORT for license detection.  
  - **Technical Details**:  
    - Implemented a scanner class that invokes the Nomos binary, parses its JSON output, and maps results into ORT’s license model.  
    - Updated the ORT Docker image to bundle the Nomos binary, enabling out-of-the-box usage without extra setup.  
    - Added a functional test to ensure correct Nomos integration inside Docker.  
  - **Impact**: Enhanced ORT’s scanning capabilities with FOSSology’s license detection while making the integration reproducible and user-friendly via Docker.
